...
class TableEvaluator:
    ...
    def plot_cumsums(self, nr_cols=4, fname=None):
        ...
        if not self.real.select_dtypes(include=['object']).empty:
            ...
            max_len = max(lengths)
        print('nr_rows ',nr_rows,' nr_cols ',nr_cols)
    ...
    def plot_distributions(self, nr_cols=3, fname=None):
        ...
        for i, col in enumerate(self.real.columns):
            if col not in self.categorical_columns:
                ...
                plot_df = pd.concat([
                        pd.DataFrame({col: self.real[col], 'kind': 'real'}),
                        pd.DataFrame({col: self.fake[col], 'kind': 'fake'})
                    ])
    ...
    def plot_correlation_difference(self, plot_diff=True, fname=None, **kwargs):
        ...
        """
        real_corr = self.real.corr()
        fake_corr = self.fake.corr()
        ...
        sns.heatmap(real_corr, cmap="YlGnBu", ax=ax1, cbar=False)
        
        ax2.title.set_text('Fake Data Distribution')
        sns.heatmap(fake_corr, cmap="YlGnBu", ax=ax2, cbar=False)

        ax3.title.set_text('Difference b/w Real & Fake Data')
        sns.heatmap(difference_corr, cmap="YlGnBu", ax=ax3, cbar=True)
